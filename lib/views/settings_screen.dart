  ...
  void _showAddFriendDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: TextField(controller: controller, decoration: const InputDecoration(labelText: 'Name der Person')),
        actions: [
  ...
  void _showDeleteConfirm() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Bist du sicher? Alle lokalen Daten dieses Tresors (inkl. Salt-Datei) werden unwiderruflich von diesem Gerät entfernt.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Abbrechen')),
          TextButton(
            onPressed: () async {
          ...
          : SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Punkt 5: Pfadanzeige (ReadOnly)
                  TextField(
                    controller: TextEditingController(text: viewModel.vaultStoragePath),
                    readOnly: true,
                    decoration: const InputDecoration(
                      labelText: 'Speicherort (Tresor & Salt)', 
                      border: OutlineInputBorder(), 
                      prefixIcon: Icon(Icons.folder_open),
                      helperText: 'Hinweis: Zum Ändern ist eine zukünftige Version mit GuardService nötig.'
                    ),
                  ),
                    ...
                    child: ElevatedButton.icon(
                      ...
                      onPressed: () {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Diese Funktion wird in einer zukünftigen Version implementiert (TODO).'))
                        );
                      },
                  ...
                  if (viewModel.friends.isEmpty)
                    const Padding(padding: EdgeInsets.all(16), child: Text('Keine weiteren Personen im Tresor.', style: TextStyle(fontStyle: FontStyle.italic)))
                  else
                    ...viewModel.friends.map((f) => Card(
                      child: ListTile(
                        title: Text(f.name),
                  ...
                  const Text('Länge', style: TextStyle(fontSize: 12, color: Colors.grey)),
                  Row(
                    children: [
                      Expanded(
                        child: Slider(
                      ...
                      SizedBox(width: 40, child: Text(viewModel.pwLength.toString(), style: const TextStyle(fontWeight: FontWeight.bold))),
                    ],
                  ),
                  const SizedBox(height: 16),
                  Row(
                    children: [
                      ...
                      Tooltip(message: 'Standard', child: IconButton(icon: const Icon(Icons.star_outline), onPressed: () { viewModel.setSpecialChars('Standard'); _specialCharsController.text = viewModel.pwSpecialCharSet; })),
                      Tooltip(message: 'Alle', child: IconButton(icon: const Icon(Icons.all_inclusive), onPressed: () { viewModel.setSpecialChars('All'); _specialCharsController.text = viewModel.pwSpecialCharSet; })),
                      Tooltip(message: 'Keine', child: IconButton(icon: const Icon(Icons.remove_circle_outline), onPressed: () { viewModel.setSpecialChars('None'); _specialCharsController.text = ''; })),
                    ],
                  ),
                  SwitchListTile(
                    title: const Text('Lesbarkeit optimieren'),
                    subtitle: const Text('Vermeidet verwechselbare Zeichen wie I, l, O, 0'),
