...
/// Sub-Escaping: `\` → `\\`, `|` → `\|`, `;` → `\;`
///
class FamKeyZipParser implements Parser {
  final AppFile _file;
  final String? _password;

  FamKeyZipParser(this._file, {String? password})
  ...
  Future<ParsedPayload> parse() async {
    ...
    if (csvEntry == null) {
      throw ParserError('Die FamKey-Exportdatei ist ungültig. `export.csv` fehlt im Archiv.', path: _file.path);
    }
    ...
    if (!_headersMatch(rows.first, expectedHeader)) {
      throw ParserError('Die FamKey-Exportdatei ist ungültig. `export.csv` hat einen unbekannten Header.',
    ...
    for (int i = 1; i < rows.length; i++) {
      ...
      if (row.length != expectedHeader.length) {
        final msg = 'Die FamKey-Exportdatei ist ungültig. Zeile ${i + 1} enthält nicht die erwartete Anzahl Felder.';
