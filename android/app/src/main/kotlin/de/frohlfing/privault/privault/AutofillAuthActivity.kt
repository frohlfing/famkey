 ...
 * zurückzugeben.
 *
 * PriVault hat aber ein besonderes Bedürfnis: Die eigentliche Flutter-UI
 * (MainActivity) soll in PriVaults eigenem Task (Aufgaben-Stack) laufen.
 ...
 * 2. Startet MainActivity mit `FLAG_ACTIVITY_NEW_TASK` (PriVaults Task)
 ...
 * PriVault hat seinen eigenen Task. Mit `FLAG_ACTIVITY_NEW_TASK` startet eine
...
class AutofillAuthActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Diese wurden von PriVaultAutofillService in den PendingIntent gepackt.
        ...
        AutofillResultRelay.set(
            onResult = { username, password ->
                ...
                val dataset = PriVaultAutofillService.buildDataset(packageName, usernameId, passwordId, username, password)
            ...
            onCancel = {
                // Dieser Block wird aufgerufen, wenn der Nutzer den X-Button in PriVault drückt.
        ...
        // PriVaults Task in den Vordergrund bringen.
        ...
        // Mit REORDER_TO_FRONT wird der PriVault-Task nach vorne gebracht, falls er
        // bereits existiert (Warm-Start). MainActivity.onNewIntent() wird dann aufgerufen.
        val mainIntent = Intent(this, MainActivity::class.java).apply {
            action = PriVaultAutofillService.ACTION_AUTOFILL_PICK
            putExtra(PriVaultAutofillService.EXTRA_DOMAIN, domain)
            putExtra(PriVaultAutofillService.EXTRA_USERNAME_ID, usernameId)
            putExtra(PriVaultAutofillService.EXTRA_PASSWORD_ID, passwordId)
